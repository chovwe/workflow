...
class Patient
{
    ...
    public function insert_patient($post)
    {
        ...
        // SQL
        $sql = " INSERT INTO ".self::$table_name." (id_alias, title, first_visit_date, last_visit_date, visit_counter, first_name, middle_name, surname, gender, address, email, phone_1, phone_2, date_of_birth, marital_status, religion, occupation, patient_type, blood_type, rh, account_status, assigned_doctor_id, country, ref_hospital_id, ref_doctor_id)
        ...
        if ($status === true)
        {
            ...
            $post['patient_id'] = $this->database_obj->last_insert_id();
            if ($this->kinship_obj instanceof Kinship)
            {
                // Insert the Kinship (Next of Kin)
                $this->kinship_obj->set_kinship_vars($post);
                $status = $this->kinship_obj->insert_kinship();
            }
            else
            {
                $status = false;
            ...
            if ($status === true)
            {
                if ($this->session_obj instanceof Session)
                {
                    ...
                    $this->session_obj->set_patient_id($post['patient_id']);
    ...
    public function update_patient($post)
    {
        ...
        {            
            if ($this->kinship_obj instanceof Kinship)
            {
                // Update the Kinship (Next of Kin)
                $kin    = $this->kinship_obj->fetch_kin_by_patient_id($patient_id);
                if(isset($kin->kinship_id))
                {
                    $this->kinship_obj->set_kinship_vars($post);
                    $status = $this->kinship_obj->update_kinship();
                }else{
                    $post['patient_id'] = $patient_id;
                    $this->kinship_obj->set_kinship_vars($post);
                    $this->kinship_obj->insert_kinship();
                }
            }
            else
            {
